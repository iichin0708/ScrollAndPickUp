...
void HelloWorld::update(float dt)
{
    ...
    for(int i = 0; i < ENEMY_NUM; i++) {
        ...
        // 画面端に行くと消える（落ちる）
        if (enemys[i]->getBody()->GetPosition().x * PTM_RATIO > field->getPosition().x + field->width / 2  + enemys[i]->width/3||
            enemys[i]->getBody()->GetPosition().x * PTM_RATIO < field->getPosition().x - field->width / 2 - enemys[i]->width/3) {
